...
quizButton.addEventListener('click', () => {
  ...
  //confirm is a boolean, hitting cancel will close/false
    const confirmation = confirm(`Hello ${name}. Are you sure, I mean really sure, you want to take the quiz? If not no worries, just click cancel and this promt will go away. Otherwise click OK and LETS DO THIS!`);
  ...
  //once done with the test, it prints the result
    if (count < 3) {
        total.textContent = `${name}, you didn't get a perfect score and have a cheat sheet! Click Take Quiz to try again!`;
    } else {
        total.textContent = `${name}, You got 100%!`;
    }
